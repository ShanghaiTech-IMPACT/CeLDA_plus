...
class Masked_Modeling(nn.Module):
    def __init__(self, landmark_num=46, feature_dim=224):
        super(Masked_Modeling, self).__init__()
        # Input shape: [landmark_num, batch_size, feature_dim]
        self.landmark_num = landmark_num
        self.feature_dim = feature_dim
        
        self.encoder = TransformerEncoder(
            TransformerEncoderLayer(d_model=feature_dim, nhead=8, dim_feedforward=1024), 
            num_layers=2
        )
        self.positional_embedding = PositionEmbeddingLearned1D(dic_len=landmark_num, d_model=feature_dim)
        self.xy_projection = nn.Linear(2, feature_dim)
    
    def random_mask(self, x, mask_num=1):
        """Randomly mask landmarks for masked modeling training."""
        mask = torch.zeros_like(x, device=x.device)
        mask_idx = torch.randperm(x.size(0), device=x.device)[:mask_num]
        mask[mask_idx, :, :] = 1
        x = x * (1 - mask)
